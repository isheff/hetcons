...
instance forall v . (Value v, Show v, Eq v, Hashable v, Parsable (Hetcons_Transaction (Participant_State v) v v)) => Hetcons_Participant_Iface (Participant v) where
  ...
  proposal_1a participant message witness
    = do { (verified :: (Verified (Recursive_1a v))) <- run_Hetcons_Transaction_IO participant (error "this should never be called") witness $ verify message -- TODO: this doesn't need a TX
  ...
  phase_1b participant message witness
    = do { (verified :: (Verified (Recursive_1b v))) <- run_Hetcons_Transaction_IO participant on_consensus witness $ verify message -- TODO: this doesn't need a TX
         ; putStrLn "Fern received 1b. This code is literally after verification."
         ; putStrLn $ "    unverified message has string length " ++ (show $ length $ show message)
         ; putStrLn $ "    unverified witness has string length " ++ (show $ length $ show witness)
         ; putStrLn $ "    verified message has string length " ++ (show $ length $ show verified)
         ; delay_message verified
