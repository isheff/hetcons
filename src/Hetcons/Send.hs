...
sign_and_verify :: (Monad_Verify b (Hetcons_Transaction s v), Encodable a, Parsable b, Hetcons_State s, Recursive a b) => a -> Hetcons_Transaction s v (Verified b)
...
instance {-# OVERLAPPABLE #-} (Value v, Hetcons_State s, Receivable s v a, Add_Sent a) => Sendable s v a where
  send m = do { receive m
              ; add_sent m}
...
instance {-# OVERLAPPING #-} (Value v, Receivable (Participant_State v) v (Verified (Recursive_1a v))) => Sendable (Participant_State v) v (Proposal_1a v) where
...
instance {-# OVERLAPPING #-} (Value v, Receivable (Participant_State v) v (Verified (Recursive_1b v))) => Sendable (Participant_State v) v (Phase_1b v) where
...
instance {-# OVERLAPPING #-} (Value v, Receivable (Participant_State v) v (Verified (Recursive_2a v))) => Sendable (Participant_State v) v (Phase_2a v) where
...
instance {-# OVERLAPPING #-} (Value v) => Sendable (Participant_State v) v (Phase_2b v) where
...
instance {-# OVERLAPPING #-} (Value v) => Sendable (Participant_State v) v (Proof_of_Consensus v) where
...
--   Also, they shouldn't be sending 1as, but whatever...
instance {-# OVERLAPPING #-} (Value v) => Sendable (Observer_State v) v (Proposal_1a v) where
  send m = do { (verified :: Verified (Recursive_1a v)) <- sign_and_verify m
              ; add_sent verified}

-- | Observers can't receive 1bs, so this send won't run a receive
--   Also, they shouldn't be sending 1bs, but whatever...
instance {-# OVERLAPPING #-} (Value v) => Sendable (Observer_State v) v (Phase_1b v) where
  send m = do { (verified :: Verified (Recursive_1b v)) <- sign_and_verify m
              ; add_sent verified}

-- | Observers can't receive 2as, so this send won't run a receive
--   Also, they shouldn't be sending 2as, but whatever...
instance {-# OVERLAPPING #-} (Value v) => Sendable (Observer_State v) v (Phase_2a v) where
  send m = do { (verified :: Verified (Recursive_2a v)) <- sign_and_verify m
              ; add_sent verified}

-- | Observers can receive 2bs, so this send will run a receive
instance {-# OVERLAPPING #-} (Value v, Receivable (Observer_State v) v (Verified (Recursive_2b v))) => Sendable (Observer_State v) v (Phase_2b v) where
  send m = do { (verified :: Verified (Recursive_2b v)) <- sign_and_verify m
              ; send verified}

-- | Observers can receive Proof_of_Consensus, so this send will run a receive
instance {-# OVERLAPPING #-} (Value v, Receivable (Observer_State v) v (Verified (Recursive_Proof_of_Consensus v))) =>
                             Sendable (Observer_State v) v (Proof_of_Consensus v) where
  send m = do { (verified :: Verified (Recursive_Proof_of_Consensus v)) <- sign_and_verify m
              ; send verified}
