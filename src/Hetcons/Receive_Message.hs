...
data (Value v, Hetcons_State s) => Hetcons_Server s v = Hetcons_Server {
...
data (Hetcons_State s) => Hetcons_Transaction_Environment s v = Hetcons_Transaction_Environment {
...
data (Hetcons_State s) => Hetcons_Transaction_State s v = Hetcons_Transaction_State {
...
instance (Hetcons_State s) => MonadError Hetcons_Exception (Hetcons_Transaction s v) where
...
instance (Hetcons_State s) => MonadRandom (Hetcons_Transaction s v) where
...
memoize :: (Eq a, Hashable a, Hetcons_State s) => (a -> (Hetcons_Transaction s v b)) -> ((Hetcons_Server s v) -> (CMap.Map a b)) -> (a -> (Hetcons_Transaction s v b))
...
instance {-# OVERLAPPING #-} (Hetcons_State s) => Monad_Verify (Recursive_1a v) (Hetcons_Transaction s v) where
  verify = memoize verify' hetcons_Server_verify_1a

-- | Memoization for verifying 1Bs in a Hetcons Transaction
instance {-# OVERLAPPING #-} (Hetcons_State s) => Monad_Verify (Recursive_1b v) (Hetcons_Transaction s v) where
  verify = memoize verify' hetcons_Server_verify_1b

-- | Memoization for verifying 2As in a Hetcons Transaction
instance {-# OVERLAPPING #-} (Hetcons_State s) => Monad_Verify (Recursive_2a v) (Hetcons_Transaction s v) where
  verify = memoize verify' hetcons_Server_verify_2a

-- | Memoization for verifying 2Bs in a Hetcons Transaction
instance {-# OVERLAPPING #-} (Hetcons_State s) => Monad_Verify (Recursive_2b v) (Hetcons_Transaction s v) where
  verify = memoize verify' hetcons_Server_verify_2b

-- | Memoization for verifying Proof_of_Consensus in a Hetcons Transaction
instance {-# OVERLAPPING #-} (Hetcons_State s) => Monad_Verify (Recursive_Proof_of_Consensus v) (Hetcons_Transaction s v) where
  verify = memoize verify' hetcons_Server_verify_proof

-- | Memoization for verifying Quorums in a Hetcons Transaction
instance {-# OVERLAPPING #-} (Hetcons_State s) => Monad_Verify_Quorums (Hetcons_Transaction s v) where
...
get_Hetcons_Transaction_State :: (Hetcons_State s) => Hetcons_Transaction s v (Hetcons_Transaction_State s v)
...
put_Hetcons_Transaction_State :: (Hetcons_State s) => (Hetcons_Transaction_State s v) -> Hetcons_Transaction s v ()
...
update_Hetcons_Transaction_State :: (Hetcons_State s) => ((Hetcons_Transaction_State s v) -> (a, (Hetcons_Transaction_State s v))) -> Hetcons_Transaction s v a
...
update_state :: (Hetcons_State s) => (s -> (a, s)) -> Hetcons_Transaction s v a
...
get_my_crypto_id :: (Hetcons_State s) => Hetcons_Transaction s v Crypto_ID
...
get_my_private_key :: (Hetcons_State s) => Hetcons_Transaction s v ByteString
...
--   You can also throw a Hetcons_Exception.
--   If an exception is thrown, it will be thrown in the IO monad, and NO STATE CHANGES WILL OCCUR, NO MESSAGES WILL BE SENT
run_Hetcons_Transaction_IO :: (Hetcons_State s) => (Hetcons_Server s v) -> ((Verified (Recursive_Proof_of_Consensus v)) -> IO ()) -> (Hetcons_Transaction s v a) -> IO a
...
--   This is intended to be used from within the `send` function.
--   Most of the time, you'll want to use `send`, which may have stuff to check to ensure everything's going correctly.
instance Add_Sent (Verified (Recursive_1a v)) v where
...
--   This is intended to be used from within the `send` function.
--   Most of the time, you'll want to use `send`, which may have stuff to check to ensure everything's going correctly.
instance Add_Sent (Verified (Recursive_1b v)) v where
...
--   This is intended to be used from within the `send` function.
--   Most of the time, you'll want to use `send`, which may have stuff to check to ensure everything's going correctly.
instance Add_Sent (Verified (Recursive_2a v)) v where
...
--   This is intended to be used from within the `send` function.
--   Most of the time, you'll want to use `send`, which may have stuff to check to ensure everything's going correctly.
instance Add_Sent (Verified (Recursive_2b v)) v where
...
--   This is intended to be used from within the `send` function.
--   Most of the time, you'll want to use `send`, which may have stuff to check to ensure everything's going correctly.
instance Add_Sent (Verified (Recursive_Proof_of_Consensus v)) v where
